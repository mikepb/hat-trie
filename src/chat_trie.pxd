...
cdef extern from "../hat-trie/src/hat-trie.h":
    ...
    int hattrie_del(hattrie_t*, const char* key, size_t len)

    ctypedef struct hattrie_iter_t:
        pass

    hattrie_iter_t* hattrie_iter_begin     (const hattrie_t*, bint sorted)
    ...
    bint            hattrie_iter_equal     (const hattrie_iter_t* a,
